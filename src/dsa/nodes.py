...
def sort_chat_inputs_first(self, vertices_layers: list[list[str]]) -> list[list[str]]:
    ...
    # First loop to gather all chat inputs and check dependencies
    for layer in vertices_layers:
        ...
        for vertex_id in layer:
            if "ChatInput" in vertex_id:
                ...
                filtered_layer.append(vertex_id)
        filtered_layers.append(filtered_layer)

    if not chat_inputs_first:
        return vertices_layers

    # Only combine chat_inputs_first with filtered layers if chat_inputs_first is not empty
